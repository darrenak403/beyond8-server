    ...
    IPublishEndpoint publishEndpoint) : ICourseReviewService
{
    public async Task<ApiResponse<CourseReviewResponse>> CreateCourseReviewAsync(CreateCourseReviewRequest request, Guid userId)
    {
        try
        {
            ...
            {
                logger.LogWarning("User {UserId} has not completed course {CourseId} (no certificate yet)", userId, request.CourseId);
                return ApiResponse<CourseReviewResponse>.FailureResponse("Bạn cần hoàn thành khóa học và có chứng chỉ trước khi đánh giá.");
            }
